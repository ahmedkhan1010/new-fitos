...
// In-memory cache — parse localStorage once, then keep in RAM
let _stateCache = null;
function loadLocalState() {
  if (_stateCache) return _stateCache;
  try {
    const raw = localStorage.getItem(LOCAL_KEY);
    _stateCache = raw ? { ...makeEmptyState(), ...JSON.parse(raw) } : makeEmptyState();
  } catch {
    _stateCache = makeEmptyState();
  }
  return _stateCache;
}
...
/* ── Exercise Difficulty Chains ────────────────────────────────
   Each entry has ordered difficulty levels: easiest → hardest.
   Used for in-workout "Too hard" / "Too easy" substitutions
   without needing to rebuild the whole plan.                   */
const DIFFICULTY_CHAINS = {
  // Push / Chest
  'Push-up':               ['Wall Push-up','Incline Push-up','Knee Push-up','Push-up','Archer Push-up','Dips / Push-up'],
  'Dips / Push-up':        ['Knee Push-up','Push-up','Dips / Push-up'],
  'Close-Grip Push-up':    ['Knee Push-up','Push-up','Close-Grip Push-up'],
  'Pike Push-up':          ['Shoulder Tap Plank','Incline Pike Push-up','Pike Push-up','Decline Push-up'],
  'Bench Press':            ['Push-up','Dumbbell Bench Press','Bench Press'],
  'Dumbbell Bench Press':   ['Push-up','Dumbbell Bench Press','Bench Press'],
  'Incline Dumbbell Press': ['Incline Push-up','Incline Dumbbell Press','Bench Press'],
  'Overhead Press':         ['Pike Push-up','Dumbbell Shoulder Press','Overhead Press'],
  'Dumbbell Shoulder Press':['Pike Push-up','Dumbbell Shoulder Press','Overhead Press'],

  // Pull
  'Inverted Row':           ['Dead Hang','Incline Inverted Row','Inverted Row','Inverted Row (feet elevated)'],
  'Pull-up / Pulldown':     ['Dead Hang','Band-Assisted Pulldown','Lat Pulldown','Pull-up / Pulldown'],
  'Lat Pulldown':           ['Band-Assisted Pulldown','Lat Pulldown','Pull-up / Pulldown'],
  'Row':                    ['Resistance Band Row','Dumbbell Row','Row','Barbell Row'],
  'Barbell Row':            ['Resistance Band Row','Dumbbell Row','Barbell Row'],
  'Dumbbell Row':           ['Resistance Band Row','Dumbbell Row','Barbell Row'],

  // Squat / Legs
  'Bodyweight Squat':       ['Sit-to-Stand (chair)','Box Squat','Bodyweight Squat','Bodyweight Squat (pause)'],
  'Goblet Squat':            ['Bodyweight Squat','Goblet Squat','Back Squat'],
  'Back Squat':               ['Bodyweight Squat','Goblet Squat','Back Squat'],
  'Squat':                    ['Bodyweight Squat','Goblet Squat','Squat'],
  'Bulgarian Split Squat':    ['Split Squat','Bulgarian Split Squat','Bulgarian Split Squat (weighted)'],
  'Split Squat':               ['Reverse Lunge','Split Squat','Bulgarian Split Squat'],
  'Leg Press':                 ['Bodyweight Squat','Leg Press'],

  // Hinge
  'Glute Bridge':              ['Glute Squeeze (lying)','Glute Bridge','Single-Leg Glute Bridge'],
  'Single-Leg Glute Bridge':   ['Glute Bridge','Single-Leg Glute Bridge'],
  'Hip Thrust':                 ['Glute Bridge','Single-Leg Glute Bridge','Hip Thrust'],
  'Romanian Deadlift':          ['Glute Bridge','Hip Thrust','Romanian Deadlift','Deadlift'],
  'Deadlift':                   ['Glute Bridge','Romanian Deadlift','Deadlift'],
  'Nordic Curl (assisted)':      ['Glute Bridge','Single-Leg Glute Bridge','Nordic Curl (assisted)'],

  // Core
  'Plank':                      ['Knee Plank','Plank (30s)','Plank','Plank (60s+)'],
  'Dead Bug':                    ['Bent Knee Dead Bug','Dead Bug','Dead Bug (arms only)'],
  'Side Plank':                  ['Side-Lying Hip Abduction','Side Plank (knee)','Side Plank'],
  'Ab Wheel / Rollout':           ['Plank','Dead Bug','Ab Wheel / Rollout'],
  'Hanging Knee Raise':           ['Dead Bug','Lying Knee Raise','Hanging Knee Raise'],
};

/* Given an exercise name and direction ('easier'|'harder'),
   return the next exercise in the difficulty chain, or null if already at limit. */
function getExerciseSubstitute(name, direction, equipArr) {
  // Find which chain this exercise lives in
  let chain = DIFFICULTY_CHAINS[name];
  if (!chain) {
    // Try to find it as a value in another chain
    for (const [key, arr] of Object.entries(DIFFICULTY_CHAINS)) {
      if (arr.includes(name)) { chain = arr; break; }
    }
  }
  if (!chain) return null;

  const idx = chain.lastIndexOf(name);
  if (idx === -1) return null;

  const targetIdx = direction === 'easier' ? idx - 1 : idx + 1;
  if (targetIdx < 0 || targetIdx >= chain.length) return null;

  const targetName = chain[targetIdx];

  // Check if EXERCISE_DB has it; if so, apply equipment check
  const dbEntry = EXERCISE_DB[targetName];
  if (dbEntry && equipArr?.length) {
    if (!userHasEquipment(equipArr, dbEntry.equip)) {
      // Try one step further in same direction
      const nextIdx = direction === 'easier' ? targetIdx - 1 : targetIdx + 1;
      if (nextIdx >= 0 && nextIdx < chain.length) {
        const nextName = chain[nextIdx];
        const nextEntry = EXERCISE_DB[nextName];
        if (nextEntry && userHasEquipment(equipArr, nextEntry.equip)) {
          return { name: nextName, sets: nextEntry.sets, reps: nextEntry.reps, rest: nextEntry.rest, notes: nextEntry.notes || '' };
        }
      }
      return null;
    }
    return { name: dbEntry.name, sets: dbEntry.sets, reps: dbEntry.reps, rest: dbEntry.rest, notes: dbEntry.notes || '' };
  }

  // Not in DB — return a placeholder (will show name only)
  return { name: targetName, sets: 3, reps: '10-12', rest: 60, notes: '' };
}

/* Expose globally so workout-current.html can call it */
window.getExerciseSubstitute = getExerciseSubstitute;
window.DIFFICULTY_CHAINS = DIFFICULTY_CHAINS;
...
const PLAN_TEMPLATES = {
  'Bodyweight Full Body': [
    { day:'Day A', exercises:[ 'Bodyweight Squat','Push-up','Inverted Row','Pike Push-up','Glute Bridge','Plank' ] },
    { day:'Day B', exercises:[ 'Bulgarian Split Squat','Dips / Push-up','Inverted Row','Push-up','Single-Leg Glute Bridge','Dead Bug' ] },
    { day:'Day C', exercises:[ 'Bodyweight Squat','Close-Grip Push-up','Inverted Row','Pike Push-up','Nordic Curl (assisted)','Side Plank' ] },
  ],
  'Full Body': [
...
function generatePlan(profile = {}) {
  ...
  const level = String(profile.fitnessLevel || 'Beginner (some experience)');
  const duration = String(profile.workoutDuration || '60 minutes');
  const diet = String(profile.diet || 'Omnivore (everything)');
  const lifestyle = String(profile.lifestyle || 'Lightly Active (some walking)');
  ...
  // Goal-aware split: muscle building prefers PPL/Upper-Lower, fat loss prefers full body + cardio
  const isMuscleBuild = /build muscle|strength|athletic/i.test(primaryGoal);
  const isFatLoss = /fat|lose|endurance/i.test(primaryGoal);
  const isBeginner = /beginner|complete/i.test(level);
  const isAdvanced = /advanced|elite/i.test(level);

  let splitType = 'Full Body';
  if (days <= 3) {
    splitType = 'Full Body'; // 3x full body = optimal for most
  } else if (days === 4) {
    splitType = isMuscleBuild && !isBeginner ? 'Upper/Lower' : 'Full Body';
  } else if (days === 5) {
    splitType = isBeginner ? 'Full Body' : (isMuscleBuild ? 'Hybrid 5-Day' : 'Upper/Lower');
  } else if (days >= 6) {
    splitType = isAdvanced ? 'Push/Pull/Legs' : 'Hybrid 5-Day';
  }

  // ── Smart rest period adjustment ─────────────────────────
  // Based on goal and level
  function targetRest(baseRest) {
    if (isFatLoss) return Math.max(45, baseRest - 30);  // shorter rest = more metabolic
    if (isMuscleBuild && isAdvanced) return baseRest + 30; // longer rest for strength
    return baseRest;
  }

  // ── Sets & reps tuning based on goal ────────────────────
  function tuneExercise(ex) {
    const e = { ...ex };

    // Rep range by goal
    if (primaryGoal === 'Build Muscle') {
      // 8-12 rep range, 3-4 sets
      if (/^[1-3]$/.test(e.sets)) e.sets = '3';
      if (/^[12]-[34]$/.test(e.reps)) e.reps = '8-12';
    } else if (primaryGoal === 'Increase Strength') {
      // 3-6 reps, 4-5 sets
      e.sets = '5'; e.reps = '3-5';
      e.rest = String(Math.min(180, parseInt(e.rest||90) + 60));
    } else if (primaryGoal === 'Lose Fat') {
      // 12-20 reps, minimal rest
      if (!/cardio|hiit/i.test(e.name)) {
        if (/^[13]-[46]$/.test(e.reps)) e.reps = '12-15';
      }
      e.rest = String(Math.max(30, parseInt(e.rest||90) - 30));
    } else if (primaryGoal === 'Improve Endurance') {
      e.reps = '15-20'; e.sets = '3';
      e.rest = '30';
    } else if (primaryGoal === 'Improve Mobility') {
      e.reps = '10-15'; e.sets = '2';
      e.rest = '45';
    }

    // Gender-based tuning (females typically benefit from higher rep ranges)
    if (gender === 'Female' && primaryGoal !== 'Increase Strength') {
      const curReps = e.reps;
      // If reps is a range like "6-8", shift it up slightly
      if (/^\d+-\d+$/.test(curReps)) {
        const [lo, hi] = curReps.split('-').map(Number);
        if (hi <= 10) e.reps = `${lo+2}-${hi+3}`;
      }
    }

    // Age adjustments: 40+ = longer rest, more mobility
    if (age >= 40) {
      e.rest = String(Math.min(150, parseInt(e.rest||90) + 15));
      if (!e.notes) e.notes = 'Take extra time warming up at 40+. Full warm-up reduces injury risk.';
    }

    // Young adults / teens: cap intensity, no max-effort sets
    if (age <= 20 && primaryGoal === 'Increase Strength') {
      e.sets = String(Math.min(3, parseInt(e.sets||3)));
      e.reps = '8-10';
      if (!e.notes) e.notes = 'Focus on perfect form over heavy weight. Strength base first.';
    }

    // Duration-based: 30min = fewer sets
    if (duration === '30 minutes') {
      e.sets = String(Math.max(2, parseInt(e.sets||3) - 1));
    } else if (duration === '90+ minutes' && isAdvanced) {
      e.sets = String(Math.min(5, parseInt(e.sets||3) + 1));
    }

    e.rest = String(targetRest(parseInt(e.rest||90)));
    return e;
  }

  const template = PLAN_TEMPLATES[splitType] || PLAN_TEMPLATES['Full Body'];
  // For bodyweight-only, always use dedicated Bodyweight Full Body template
  const isBodyweightOnly = equipArr.length === 1 && equipArr[0] === 'Bodyweight Only';
  if (isBodyweightOnly) splitType = 'Bodyweight Full Body';
  const effectiveTemplate = isBodyweightOnly ? PLAN_TEMPLATES['Bodyweight Full Body'] : template;

  const weeks = effectiveTemplate.slice(0, days).map(day => ({
    day: day.day,
    exercises: day.exercises
      .map(name => resolveExercise(name, equipArr, badJoints, postSurgery))
      .filter((ex, idx, arr) => {
        // De-duplicate: skip exercises with same name as a previous one in this day
        return arr.findIndex(e => e.name === ex.name) === idx;
      })
      .map(ex => tuneExercise(ex)),
  }));
  ...
  if (isBeginner) {
    weeks.forEach(w => w.exercises.forEach(e => {
      if (!e.notes || e.notes.trim() === '') {
        e.notes = 'New to this movement? Start with 50-60% of what you think your max is. Master form first, then add weight.';
      }
    }));
  }

  // ── Lifestyle energy advice ──────────────────────────────
  const lifestyleNote = lifestyle.includes('Sedentary')
    ? 'You have a sedentary lifestyle — try to walk 8,000 steps daily between workout sessions.'
    : lifestyle.includes('Very Active') || lifestyle.includes('Extremely')
    ? 'With your active lifestyle, prioritize recovery and sleep. Protein and sleep matter as much as training.'
    : '';

  // ── Diet-based notes ────────────────────────────────────
  let dietNote = '';
  if (/vegan|vegetarian/i.test(diet)) dietNote = 'As a plant-based eater, combine rice + lentils or tofu + edamame for complete amino acid profiles. Consider B12 and creatine supplements.';
  else if (/keto/i.test(diet)) dietNote = 'Keto training tip: your first 2-3 weeks may feel harder. Electrolytes (sodium, potassium, magnesium) are critical.';
  else if (/intermittent/i.test(diet)) dietNote = 'Try to schedule workouts within your eating window for best performance and recovery.';
  ...
  return {
    ...
    weeks,
    progressionRules: {
      type: isMuscleBuild ? 'doubleProgression' : 'linearProgression',
      upperIncrement: primaryGoal === 'Increase Strength' ? 2.5 : 1.25,
      lowerIncrement: primaryGoal === 'Increase Strength' ? 5 : 2.5,
      deloadEveryWeeks: isAdvanced ? 4 : 6,
    },
    coachNotes: [
      `Plan created for: ${primaryGoal} · ${level} · ${days} days/week`,
      lifestyleNote, dietNote,
      injuriesArr.filter(i => i !== 'No injuries').length
        ? `⚠️ Working around: ${injuriesArr.filter(i => i !== 'No injuries').join(', ')}`
        : '',
    ].filter(Boolean),
    reasonForCreation: [
      `Intelligently generated for ${primaryGoal}, ${level}, ${days} days/week, ${duration} sessions.`,
      `Equipment: ${equipArr.join(', ')}.`,
      injuriesArr.filter(i=>i!=='No injuries').length ? `Avoiding exercises that load: ${injuriesArr.filter(i=>i!=='No injuries').join(', ')}.` : '',
      gender === 'Female' ? 'Rep ranges tuned for female physiology (higher rep capacity, hormonal recovery curves).' : '',
      age >= 40 ? 'Rest periods extended for 40+ recovery rates. Warm-up reminders added.' : '',
      age <= 20 ? 'Intensity capped for adolescent development.' : '',
    ].filter(Boolean).join(' '),
    createdAt: new Date().toISOString(),
  };
}

// XSS-safe HTML escaping for any user-provided or Firestore-sourced content
function esc(str) {
  if (str == null) return '';
  return String(str)
    .replace(/&/g, '&amp;')
...
const FS = {
  async getProfile(uid) {
    // 1. Return local state instantly if available
    const local = readLocalDoc('profiles', uid);
    if (local && local.onboardingDone) {
      // Refresh from Firestore in background without blocking
      db.doc(`profiles/${uid}`).get().then(snap => {
        if (snap.exists) writeLocalDoc('profiles', uid, { ...snap.data(), uid });
      }).catch(() => {});
      return local;
    }
    // 2. No local data — must wait for Firestore
    try {
  ...
  async getActivePlan(uid, knownProfile = null) {
    ...
    // 1. Check local state first — instant, no network
    const cachedPlanId = profile?.activePlan?.planId || profile?.cachedPlan?.planId;
    const localPlan = cachedPlanId ? readLocalDoc('workout_plans', cachedPlanId) : null;
    if (localPlan && localPlan.weeks?.length) {
      // Sync from Firestore in background
      db.collection('workout_plans').where('uid','==',uid).get().then(snap => {
        const docs = snap.docs.map(d => ({ planId: d.id, ...d.data() }))
          .filter(p => (p.status || 'active') === 'active').sort(sortByRecentDesc);
        if (docs.length) writeLocalDoc('workout_plans', docs[0].planId, docs[0]);
      }).catch(() => {});
      return { planId: localPlan.planId || cachedPlanId, ...safeClone(localPlan) };
    }

    // 2. Check profile-embedded plan (saved directly in profile doc)
    const direct = profile?.activePlan || profile?.cachedPlan;
    if (direct && direct.weeks?.length) {
      writeLocalDoc('workout_plans', direct.planId || `plan_${uid}`, direct);
      return { planId: direct.planId || `plan_${uid}`, ...safeClone(direct) };
    }

    // 3. Nothing local — fetch from Firestore
    try {
...
function calcStreak(logs) {
  ...
  const weeks = new Set(logs.map(l => {
    ...
    return `${d.getFullYear()}-${getWeekOfYear(d)}`;
  }));
  const now = new Date();
  let streak = 0;
  for (let i = 0; i < 52; i++) {
    const d = new Date(now); d.setDate(d.getDate() - i * 7);
    if (weeks.has(`${d.getFullYear()}-${getWeekOfYear(d)}`)) streak++;
    else break;
  }
  return streak;
}
